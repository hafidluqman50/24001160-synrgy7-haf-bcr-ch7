...
export default function Create(): ReactElement {
  ...
  const createDataCar = async(request: CreateFormCar): Promise<{
    status:boolean,
    message:string
  }> => {
    const response:{status:boolean, message:string} = await httpServer.post('/api/cars', request, {
      headers: {
  ...
  const { register, handleSubmit } = useForm<CreateFormCar>()
  ...
  const submit: SubmitHandler<CreateFormCar> = (data): void => {
    const car: CreateFormCar = {
      name: data.name,
    ...
    >
      <MainSection>
        <MainSectionMenuGroup title="CARS">
          <MainSectionMenu title="List Cars" to="/superadmin/cars" isActive />
          <MainSectionMenu title="Log Cars" to="/superadmin/cars/logs" />
        </MainSectionMenuGroup>
        <MainSectionContent>
          <BreadCrumb>
            <BreadCrumbMenu title="Cars"/>
            <BreadCrumbMenu title="List Cars" />
            <BreadCrumbMenu title="Create Data" isActive />
          </BreadCrumb>
          <div className="w-100">
